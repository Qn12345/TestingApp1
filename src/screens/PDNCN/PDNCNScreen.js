...
const PDNCNScreen = ({ navigation }) => {
  ...
  const filteredPDNCNData = PDNCNData.filter((PDNCN) =>
  PDNCN.refno.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        onEndReached={() => {
          if (PDNCNData.length > 99)
          {
            console.log(limit);
            console.log(poffset);
            console.log(PDNCNData.length);
...
export default withAuth(PDNCNScreen);
