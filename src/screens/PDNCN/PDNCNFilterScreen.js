...
const PDNCNFilterScreen = () => {
  ...
  const [RefNo, setPDNCN] = useState('');
  const [isPDNCNFocused, setIsPDNCNFocused] = useState(false);
  const [PDNCNStatus, setPDNCNStatus] = useState([]);
  const [PDNCNDoctype, setPDNCNDoctype] = useState([]);
  const [PDNCNFromDate, setPDNCNFromDate] = useState('');
  const [PDNCNToDate, setPDNCNToDate] = useState('');
  const [PDNCNEFromDate, setPDNCNEFromDate] = useState('');
  const [PDNCNEToDate, setPDNCNEToDate] = useState('');
  const [selectedPeriodCode, setSelectedPeriodCode] = useState('');
  const typePDNCN = 'PDNCN';
  const limit = 100;
  const offset = 0;
  ...
  const togglestatusChip = (status) => {
    setPDNCNStatus((prevPDNCNStatus) => {
      const isSelected = prevPDNCNStatus.includes(status.code);
      if (isSelected) {
        return prevPDNCNStatus.filter((selectedStatus) => selectedStatus !== status.code);
      } else {
        return [...prevPDNCNStatus, status.code];
      }
    });
  };

  const toggledocChip = (doc_type) => {
    setPDNCNDoctype((prevPDNCNDoctype) => {
      const isSelected = prevPDNCNDoctype.includes(doc_type.code);
      if (isSelected) {
        return prevPDNCNDoctype.filter((selectedDoc_type) => selectedDoc_type !== doc_type.code);
      } else {
        return [...prevPDNCNDoctype, doc_type.code];
      }
    });
  };
  ...
  const handleReset = () => {
    setPDNCN('');
    setIsPDNCNFocused(false);
    setPDNCNStatus([]);
    setPDNCNDoctype([]);
    setPDNCNFromDate(null);
    setPDNCNToDate(null);
    setPDNCNEFromDate(null);
    setPDNCNEToDate(null);
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scroll}>
        <View style={styles.inputContainer}>
          <Text style={styles.label}>P DN/CN#</Text>
          <TextInput
            style={[
              styles.input,
              {marginTop:5},
              { borderColor: isPDNCNFocused ? COLORS.LightGrey : COLORS.LightBlue },
            ]}
            value={RefNo}
            onChangeText={(text) => setPDNCN(text)}
            onFocus={() => setIsPDNCNFocused(true)}
            onBlur={() => setIsPDNCNFocused(false)}
          />
        </View>

        <View style={styles.inputContainer}>
          <Text style={styles.label}>P DN/CN Type:</Text>
          <View style={styles.chipsContainer}>
            {Ddoc_type.map((doc_type) => (
                <Chip
                containerStyle={styles.Schip}
                key={doc_type.code}
                type={PDNCNDoctype.includes(doc_type.code) ? 'solid' : 'outline'}
        ...
        <View style={styles.inputContainer}>
          <Text style={styles.label}>P DN/CN Status:</Text>
          <View style={styles.chipsContainer}>
            {statuss.map((status) => (
                <Chip
                containerStyle={styles.Schip}
                key={status.code}
                type={PDNCNStatus.includes(status.code) ? 'solid' : 'outline'}
...
export default withAuth(PDNCNFilterScreen);
