...
const GRDAScreen = ({ navigation }) => {
  ...
  if (JSON.stringify(fstatus) === JSON.stringify(["ALL"])) {
    status = 'default';
  }

  const filteredGRDAData = GRDAData.filter((GRDA) =>
    GRDA.refno.toLowerCase().includes(searchQuery.toLowerCase())
  );
    ...
    <TouchableOpacity
          ...
          >
            <ListItem.Content>
              ...
              <ListItem.Subtitle>
                <Text style={styles.blueLabel}>GRDA# </Text>
  ...
  return (
    <View style={styles.container}>
      ...
      <FlatList
        ...
        onEndReached={() => {
          if (GRDAData.length > 99)
          {
            console.log(limit);
            console.log(poffset);
            console.log(GRDAData.length);
...
export default withAuth(GRDAScreen);
