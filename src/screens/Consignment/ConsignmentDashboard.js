...
const ConsignmentDashboard = ({ navigation }) => {
  const {isLoading,displayDoc} = useContext(DisplayFunc);
  const {consignSRFilter} = useContext(ConsignFunc);
  const route = useRoute();
  const dashboardConsignData = route.params?.dashboardConsignData || [];
  ...
  return (
    ...
    <Spinner visible={isLoading} />
      <PublicHeader title={'Consignment Dashboard'}/>
      ...
      <View>
        ...
        {( dashboardConsignData !== undefined || dashboardConsignData !== null) && (
          dashboardConsignData.map((consign, index) => (
          <React.Fragment key={index}>
          <TouchableOpacity
            style={[styles.button,{ backgroundColor: `${consign.background_color}` }]}
            onPress={async () =>
                      {
                        if (consign.type === 'ConsignSR'|| consign.type === 'ConsignSRS')
                        {
                          consignSRFilter(consign.type);
                        }
                        else if (consign.type === 'ConsignSS')
                        {
                          const location = await AsyncStorage.getItem('location');
                          const ishq = await  AsyncStorage.getItem('ishq');
                          displayDoc(location,ishq,consign.type,status,refno,period_code,date_from,date_to,exp_from,exp_to,doc_type,limit,offset,filter_supplier);
                        }
                        else
                        {
                          Alert.alert('Error');
          ...
          >
            <View style={styles.buttonContent}>
              ...
              <View style={styles.NumWord}>
                <Text style={styles.buttonTextAmt}>{consign.count_doc}</Text>
                <Text style={styles.buttonText}>{consign.display_name}</Text>
              </View>
            </View>
          </TouchableOpacity>
          </React.Fragment>
...
export default withAuth(ConsignmentDashboard);
